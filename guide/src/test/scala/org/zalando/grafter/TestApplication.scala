...
object ApplicationConfig extends ApplicationConfig {
  ...
  implicit def prodReader[A]: Reader[A, ApplicationConfig] = Reader(_ => this)
}
case class HttpConfig(host: String, port: Int)

@reader
case class Application(config: ApplicationConfig)
...
val application: Application =
  Application.reader[ApplicationConfig].apply(testConfiguration).
    singletons.
